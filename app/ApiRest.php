...
abstract class ApiRest extends BaseController
{
    ...
    public function __construct(App $app){
      ...
      //
        ...
        if (isset($this->autograph)) {
            ...
            if($this->checkCoachAccountRoute()==false){

                $this->errorMsg('need login', 401);
            }

            if(!empty($this->_user['openid'])){

                $this->checkOrderPayV2($this->_user['openid'],$this->payConfig($this->_uniacid),$this->_uniacid);
            }
        }
    }
     ...
     * @DataTime: 2024-03-05 16:55
     * @功能说明:校验技师端账号登录的路由
     */
    public function checkCoachAccountRoute(){
        ...
        $url =   app('http')->getName().'/app/'.$this->_controller.'/'.$this->_action;

        $pass_url = [

            'massage/app/IndexUser/coachInfo',

            'massage/app/IndexUser/bindAlipayNumber',

            'massage/app/IndexUser/getAddressByIp',

            'massage/app/Index/expectationCityCheck',

            'massage/app/IndexUser/getStoreSelect',

            'massage/app/IndexUser/industryTypeSelect',

            'massage/app/IndexUser/stationIconSelect',

            'massage/app/IndexUser/coachIconSelect',

            'massage/app/Index/adminCarCashInfo',

            'massage/app/IndexUser/coachTypeSelect',

            'massage/app/IndexUser/industryTypeSelect',

            'massage/app/IndexGoods/goodsInfo',

            'massage/app/IndexUser/getWecomStaff',

            'hotel/app/IndexHotel/hotelInfo'
        ];

        if(in_array($url,$pass_url)){

            return true;
        }

        if(in_array($this->_controller,['IndexCoach','IndexDynamicCoach','Index','Wx','WxFile','IndexMember'])){
...
//        }
        ...
        $arr = [
            ...
            'massage/app/Index/nearbyLocation',
            'massage/app/Index/industryServiceList',
            'massage/app/Index/getServiceObjList',
            'massage/app/Index/industryGetCoachService',
            'massage/app/Index/expectationCityCheck',
            'massage/app/Index/cityData',
            'massage/app/Index/getCityList',
            ...
            'massage/app/IndexUser/getUserCoachStatus',
            'massage/app/IndexUser/getWecomStaff',
            'massage/app/IndexUser/getAddressByIp',
            'massage/app/IndexUser/addScanRecord',
            'massage/app/IndexGoods/goodsInfo',
            'map/app/Index/coachList',
            'massage/app/Index/agentInfo',
            'store/app/IndexStore/storeCateList',
            'massage/app/IndexUser/industryTypeSelect',
            'partner/app/typeList',
            'partner/app/getPartnerList',
            'store/app/IndexStore/storePackList',
            'store/app/IndexStore/storePackInfo',
            'hotel/app/IndexHotel/hotelList',
            'hotel/app/IndexHotel/hotelInfo'
        ];
...
//        }
    ...
    protected function getUserInfo()
    {
        ...
        if(!empty($value['coach_account_login'])){

            $coach_model = new Coach();

            $value['id'] = $coach_model->where(['id'=>$value['coach_id']])->value('user_id');
        }

        if(!empty($value)&&empty($value['coach_account_login'])){
     ...
     */
    //@ioncube.dk myk("sha256", "cnjdbvjdnjd") -> "cff6bcac6bd92467e0cee72e5c879cdbf7044386eda8f464c817bd5c5c963d6f" RANDOM
    public function payConfig($uniacid = '666', $is_app = 7){
    ...
    function getLatData($lng, $lat,$uniacid){
        ...
        if(!empty($data['result']['address_reference']['town']['title'])&&$data['result']['address_reference']['town']['title']=='白沟镇'){

            $arr = str_replace('高碑店市','保定白沟新城',$arr);
        }
     ...
     * @author chenniang(龙兵科技)
     * @DataTime: 2024-05-10 14:45
     */
    public function checkOrderPayV2($openid,$paymentApp,$uniacid){

        $list = pullCache($openid.'payresult',$uniacid);
        ...
        if(!empty($list)){
            ...
            foreach ($list as $value){
                ...
                if($value['type']=='Massage'){

                    $order_model = new Order();

                }elseif ($value['type']=='MassageUp'){
                ...
                //微信支付
                if($value['pay_model']=='weixin'){

                    $wxpay = new IndexWxPay($this->app);

                    $result = $wxpay->checkOrder($value['order_code'],$paymentApp);
                ...
                }elseif ($value['pay_model']=='adapay'){

                    $result = $adapay_model->orderInfo($value['order_code']);

                    if(isset($result['status'])&&$result['status']=='succeeded'&&!empty($result['id'])){
                        ...
                        if(!empty($record)){
                            ...
                            $order_model->orderResult($record['order_code'],$result['id']);
                        }
                    }
                }
                //目前只处理一条 实际正常情况也只有一条
    ...
    //实时显示人数展示
    public function showTimeCount($user_id)
    {
        $user_online=Db::name('massage_service_user_online')->select()->toArray();
        $timeout=30+time();

        $user_ip=array_column($user_online,'ip');
        //没有记录的IP直接添加
        if(!in_array($_SERVER['REMOTE_ADDR'],$user_ip)){
            Db::name('massage_service_user_online')->insert(['ip'=>$_SERVER['REMOTE_ADDR'],'create_time'=>$timeout,'user_id'=>$user_id]);
        }
        ...
        if(!empty($user_online)){
            foreach ($user_online as $user){
                ...
                if($user['ip']==$_SERVER['REMOTE_ADDR']){
                    Db::name('massage_service_user_online')->where('id','=',$user['id'])->update(['ip'=>$_SERVER['REMOTE_ADDR'],'create_time'=>$timeout,'user_id'=>$user_id]);
                }else{//如果时间过期，则删除
                    if($user['create_time']<time()){
                        Db::name('massage_service_user_online')->where('id','=',$user['id'])->delete();
                    }
                }
            }
        }
