...
Class SaasAuthConfig {
     ...
     */
    public static function getSAuthConfig (int $uniacid,$update=0): ?array
    {
        if (isset(self::$sAuthConfig[$uniacid])&&$update==0) {
            return self::$sAuthConfig[$uniacid];
        }

        try {
            $sAuthConfig = self::_getsAuthConfig($uniacid,$update);
    ...
    private static function _getsAuthConfig($uniacid, $update=0)
    {
       ...
       // dump($param_list);exit;
        ...
        if (!empty($param_list)) {
            ...
            setCache('single_checked_auth_' . $app_model_name . $domain_name, $auth_data,  86400,  $uniacid);
